...
public class JWTTokenGenerator extends OncePerRequestFilter {
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		if (auth != null) {
			...
			String jwt = Jwts.builder().setIssuer("ecom").setSubject("JWT Token").claim("userName", auth.getName())
					.claim("authorities", getAuthorities(auth.getAuthorities())).setIssuedAt(new Date())
					.setExpiration(new Date(new Date().getTime() + 6000000)).signWith(key).compact();
